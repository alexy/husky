{-# LANGUAGE DeriveDataTypeable, MagicHash, Rank2Types, CPP, NoBangPatterns #-}
-----------------------------------------------------------------------------
-- |
-- Module      :  Data.IntMap
...
instance (Binary e) => Binary (IntMap e) where
    put m = put (size m) >> mapM_ put (toAscList m)
...
instance Data a => Data (IntMap a) where
  ...
  dataTypeOf _  = mkNoRepType "IntMap.IntMap"
...
lookup k t
  = let nk = natFromInt k  in (lookupN nk t)

lookupN :: Nat -> IntMap a -> Maybe a
lookupN k tr = seq k getIt tr
  where
    getIt t
      = case t of
          Bin _ m l r
            | zeroN k (natFromInt m) -> getIt l
            | otherwise              -> getIt r
          Tip kx x
      ...
      insrt t
        = case t of
           Bin p m l r
             ...
             | zero k m      -> Bin p m (insrt l) r
             | otherwise     -> Bin p m l (insrt r)
           Tip ky _
             | k == ky      -> Tip k x
    ...
    where
      insrtW t =
        case t of
          Bin p m l r
            | nomatch k p m -> join k (Tip k x) p t
            | zero k m      -> Bin p m (insrtW l) r
            | otherwise     -> Bin p m l (insrtW r)
          Tip ky y
            | k == ky       -> Tip k (f k x y) -- let v = f k x y in v `seq` (Tip k v)
            | otherwise     -> join k (Tip k x) ky t
          Nil -> Tip k x
--   = case t of
--       Bin p m l r
    ...
    altr t =
      case t of
        Bin p m l r
          ...
          | zero k m      -> bin p m (altr l) r
          | otherwise     -> bin p m l (altr r)
        Tip ky y
...
mapWithKey f tr = mwK tr
  where
    mwK t =
      case t of
        Bin p m l r -> Bin p m (mwK l) (mwK r)
        Tip k x     -> Tip k (f k x) -- let v = f k x in v `seq` Tip k v
        Nil         -> Nil
--   = case t of
...
foldr f z t
  = case t of
      Bin 0 m l r | m < 0 -> let n = foldr' f z l in n `seq` foldr' f n r
      -- foldr' f (foldr' f z l) r  -- put negative numbers before.
      Bin _ _ _ _ -> z `seq` foldr' f z t
      Tip k x     -> f k x z
      Nil         -> z

foldr' :: (Key -> a -> b -> b) -> b -> IntMap a -> b
foldr' f z tr = fldr' z tr
  where
    fldr' w t =
      case t of
        Bin _ _ l r -> let v = fldr' w r in v `seq` fldr' v l
        Tip k x     -> f k x w
        Nil         -> w
--   = case t of
...
foldWithKeyL :: (Key -> a -> b -> b) -> b -> IntMap a -> b
foldWithKeyL f z t
  = foldrL f z t

foldrL :: (Key -> a -> b -> b) -> b -> IntMap a -> b
foldrL f z t
  = case t of
      Bin 0 m l r | m < 0 -> foldrL' f (foldrL' f z l) r  -- put negative numbers before.
      Bin _ _ _ _ -> foldrL' f z t
      Tip k x     -> f k x z
      Nil         -> z
...
foldrL' f z t
  = case t of
      Bin _ _ l r -> foldrL' f (foldrL' f z r) l
      Tip k x     -> f k x z
      Nil         -> z
...
toAscList t
  = -- NOTE: the following algorithm only works for big-endian trees
    let (pos,neg) = span (\(k,_) -> k >=0) (foldrL (\k x xs -> (k,x):xs) [] t) in neg ++ pos
...
maskW i m
  = intFromNat (i .&. (complement (m `shiftL` 1) + 1))
  --  = intFromNat (i .&. (m * complement 1))
...
--         x5 -> (x5 `xor` (shiftRL x5 1))      -- for 32 bit platforms
